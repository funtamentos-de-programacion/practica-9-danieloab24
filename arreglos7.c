...
int main() {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            printf("%d\t", *(ptr + i*3 +j)); //direccionees contiguas, (direcciones seguidas unas de otras)
        }
        printf("\n");
    }
        ...
        for (int i = 0; i < 9; i++) {
            printf("%d\t", *(ptr + i)); 
            ((i)%3==2)? printf("\n") : printf(""); //un arreglo multidimensional se guarda en forma lineal en la memoria 
        }
